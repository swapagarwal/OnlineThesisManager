     ...
     ini_set('max_execution_time', 300);
      if (!($con = mysqli_connect(constant("HOSTNAME"), constant("USERNAME"), constant("PASS")))) {
        ...
        $result = "DBCONNECTION_ERROR";
                   ...
                   $string="SELECT COUNT(*) FROM `project`.`calander` WHERE user_name='".$_SESSION['admin_user_nm']."' AND ('".$timefrom."' BETWEEN Fromtime AND Totime OR'".$timeto."'BETWEEN Fromtime AND Totime);";
                  // echo $string;
              ...
              if($row[0]==0)
                {
                     //echo "heelo";
                      ...
                      $sql = "INSERT INTO `project`.`calander` (`Totime`, `Fromtime`, `User_name`, `Content`) VALUES ("."'".$timeto."','".$timefrom."','".$_SESSION['admin_user_nm']."','".$content."')";
